...
uint32_t EnableTxEnhancedTextureFileStorage = 0;
uint32_t EnableTxHiresTextureFileStorage  = 0;
...
static void setup_variables(void)
{
    struct retro_variable variables[] = {
        ...
        { CORE_NAME "-txEnhancedTextureFileStorage",
            "Enhanced Texture FileStorage; False|True" },
        { CORE_NAME "-txHiresTextureFileStorage",
            "Enhanced High-Res Texture FileStorage; False|True" },
...
void update_variables()
{
    ...
    var.key = CORE_NAME "-txEnhancedTextureFileStorage";
    var.value = NULL;
    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
    {
        EnableTxEnhancedTextureFileStorage = !strcmp(var.value, "False") ? 0 : 1;
    }
    
    var.key = CORE_NAME "-txHiresTextureFileStorage";
    var.value = NULL;
    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
    {
        EnableTxHiresTextureFileStorage = !strcmp(var.value, "False") ? 0 : 1;
    }
